...
// die 4 byte codes sind nur g�ltig, wenn count >= 4
...
struct USART_buffer
{
 ...
 ISR(USART0_RX_vect)
 {
	...
	uint8_t RX_GetNextCommand(struct S_SER_IN* serial_in)
	{
		...
		if (rx_buffer.cnt>=8) // ev wieder auf while �ndern
		{
			if (!RX_GetNext(&chk_ff_0)) // sollte passen, da cnt>=4 abgefragt wurde
			{
				// 1. byte auf g�ltges cmd �berpr�fen
...
ISR(USART0_TX_vect)
{
	...
	{
		UDR0 = tx_buffer.data[tx_buffer.ind_read];						// n�chstes byte holen
...
void TX_SendChar(uint8_t c)
{
	...
	IRcomm_on();
	IRon();
	if ((tx_buffer.ind_write != tx_buffer.ind_read) || ((UCSR0A & (1<<UDRE0) )==0))	// Wenn daten im Buffer oder in RS232 dann an n�chste Bufferstelle schrieben
	{
